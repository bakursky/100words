    ...
    return (
        <>
            <div className="bg-neutral-900 fixed inset-0 z-40 flex flex-col w-full">
                ...
                >
                    <p className="w-64 text-center">Write at least 100 words to save your daily note! </p>
                    <div className="flex gap-2 justify-center">
                        <button onClick={() => setExitModalOpen(false)} className="text-white/80 bg-neutral-700 p-2 rounded-full mt-2">Return</button>
                        <button onClick={() => {setNewEntryOpen(false); setExitModalOpen(false); setValue(todaysNote)}} className="text-white/80 bg-red-700 p-2 px-4 rounded-full mt-2">Exit</button>
                    </div>
                </Modal>

                <textarea
                    autoFocus
                    className="bg-neutral-900 flex-1 p-4 resize-none outline-none text-lg caret-red-600 text-neutral-200 placeholder:text-neutral-600 selection:bg-neutral-700/50 scrollbar-hide"
                ...
                <button onClick={() => setExitModalOpen(true)} className='fixed top-5 right-5'>
